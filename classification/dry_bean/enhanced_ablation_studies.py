...
def save_model_ablation_results(model_name: str, results: Dict, dataset_name: str = "dry_bean", 
...
def load_model_ablation_results(model_name: str, dataset_name: str = "dry_bean"):
    """
...
def save_comprehensive_ablation_results(all_results: Dict, dataset_name: str = "dry_bean"):
    """
...
class EnhancedAblationStudyAnalyzer:
    ...
    Adapted for multi-class classification (dry bean dataset)
    """
    ...
    def _get_baseline_performance(self, model, X_train, X_test, y_train, y_test):
        """Get baseline performance metrics - adapted for multi-class classification"""
        ...
        # For multi-class classification, use macro and weighted averages
        metrics = {
            'accuracy': accuracy_score(y_test, y_pred),
            'f1_macro': f1_score(y_test, y_pred, average='macro'),
            'f1_weighted': f1_score(y_test, y_pred, average='weighted'),
            'f1': f1_score(y_test, y_pred, average='weighted'),  # Use weighted as default
        ...
        # Try to get AUC for multi-class (if predict_proba is available)
        try:
            if hasattr(model, 'predict_proba'):
                ...
                if len(np.unique(y_train)) > 2:
                    ...
                    metrics['auc_ovr'] = roc_auc_score(y_test, y_proba, multi_class='ovr', average='weighted')
    ...
    def _feature_ablation_study(self, model, model_name: str, X_train, X_test, y_train, y_test, baseline_metrics):
        """Enhanced feature ablation study - adapted for multi-class"""
        ...
        for i in range(n_features):
            ...
            try:
                ...
                f1_score_reduced = f1_score(y_test, y_pred, average='weighted')
        ...
        # Feature group removal (domain-specific groups for dry bean)
        feature_groups = self._define_feature_groups()
        for group_name, feature_indices in feature_groups.items():
            if len(feature_indices) > 0 and max(feature_indices) < n_features:
                feature_mask = np.ones(n_features, dtype=bool)
    ...
    def _define_feature_groups(self):
        """Define domain-specific feature groups for dry bean dataset"""
        # Dry bean dataset features (16 features total)
        # Based on typical dry bean feature names: Area, Perimeter, MajorAxisLength, MinorAxisLength, 
        # AspectRatio, Eccentricity, ConvexArea, EquivDiameter, Extent, Solidity, roundness, 
        # Compactness, ShapeFactor1, ShapeFactor2, ShapeFactor3, ShapeFactor4
        
        return {
            'Size_Features': [0, 1, 6, 7],  # Area, Perimeter, ConvexArea, EquivDiameter
            'Shape_Features': [2, 3, 4, 5],  # MajorAxisLength, MinorAxisLength, AspectRatio, Eccentricity
            'Geometric_Properties': [8, 9, 10],  # Extent, Solidity, Roundness
            'Shape_Factors': [11, 12, 13, 14, 15]  # Compactness, ShapeFactor1-4
        }
    ...
    def _noise_robustness_study(self, model, model_name: str, X_train, X_test, y_train, y_test, baseline_metrics):
        ...
        for noise_level in noise_levels:
            ...
            try:
                ...
                f1_noisy = f1_score(y_test, y_pred, average='weighted')
            ...
            for batch_size in batch_sizes:
                try:
                    ...
                    results.append({
                        ...
                        'f1_score': f1_score(y_test_sample, y_pred_batches, average='weighted'),
                        'inference_time': inference_time,
    ...
    def _tabicl_context_window_ablation(self, model, X_train, X_test, y_train, y_test):
        """TabICL context window utilization ablation - adapted for multi-class"""
        ...
        for strategy in window_strategies:
            try:
                if strategy == 'full':
                    ...
                    # Balance classes in context for multi-class
                    unique_classes = np.unique(y_train)
                    n_per_class = min([len(np.where(y_train == cls)[0]) for cls in unique_classes])
                    n_per_class = min(n_per_class, 16)  # Limit to 16 per class
        ...
        for config in configs:
            try:
                ...
                ft_model = rtdl.FTTransformer.make_default(
                    ...
                    cat_cardinalities=[],
                    d_out=n_classes,  # Multi-class output
                    **config
                )
    ...
    def _ft_transformer_batch_size_ablation(self, model, X_train, X_test, y_train, y_test):
        ...
        for batch_size in self.model_specific_configs['FT-Transformer']['batch_sizes']:
            try:
                ...
                results.append({
                    'batch_size': batch_size,
                    'f1_score': f1_score(y_test, all_preds, average='weighted'),
    ...
    def _generate_comprehensive_report(self, model_names: List[str]):
        ...
        for model_name in model_names:
            if model_name in self.results and 'baseline' in self.results[model_name]:
                ...
                print(f"  F1-Score (Weighted): {baseline['f1']:.4f}")
                print(f"  F1-Score (Macro): {baseline.get('f1_macro', 'N/A'):.4f}" if baseline.get('f1_macro') else f"  F1-Score (Macro): N/A")
                ...
                print(f"  Classes: {baseline.get('n_classes', 'N/A')}")
        ...
        for model_name in model_names:
            if model_name in self.results:
                ...
                # XGBoost insights
                if 'hyperparameter_ablation' in self.results[model_name]:
                    hyper_results = self.results[model_name]['hyperparameter_ablation']
                    if hyper_results:
                        best_param = max(hyper_results, key=lambda x: x['f1_score'])
                        print(f"  Best hyperparameter: {best_param['parameter']}={best_param['value']} "
                              f"(F1: {best_param['f1_score']:.4f})")
...
def run_enhanced_ablation_studies(models_dict, model_names, X_train, X_test, y_train, y_test, 
                                 feature_names=None, dataset_name="dry_bean"):
    """
    ...
    for model_name, model_results in results.items():
        ...
        if 'baseline' in model_results:
            ...
            row = {
                ...
                'F1_Macro': baseline.get('f1_macro', None),
                'F1_Weighted': baseline.get('f1_weighted', None),
                'Precision': baseline['precision'],
                'Recall': baseline['recall'],
                'AUC': baseline.get('auc', None),
                'Train_Time': baseline['train_time'],
                'Inference_Time': baseline['inference_time'],
                'N_Classes': baseline.get('n_classes', None)
            }
