...
This script loads and compares all trained models:
- XGBoost
- FT-Transformer 
- SAINT
...
try:
    ...
    logger.info(f"Data loaded: {len(X_test_scaled)} test samples, {len(feature_names)} features")
...
try:
    with open('./Section2_Model_Training/xgboost_model.pkl', 'rb') as f:
        xgb_model = pickle.load(f)
    
    # The model is saved directly as XGBRegressor, not in a dictionary
    models['XGBoost'] = xgb_model
    
    # Load metrics from CSV file
    try:
        xgb_metrics_df = pd.read_csv('./Section2_Model_Training/xgboost_evaluation_results.csv', index_col=0)
    ...
    model_results['XGBoost'] = {
        ...
        'model_size': 'N/A'  # XGBoost doesn't have parameter count like neural networks
    }
...
try:
    # Import FT-Transformer functions
    from ft_transformer_training_functions import create_ft_transformer_model
    import rtdl
    ...
    print("✅ FT-Transformer model loaded successfully!")
    logger.info("FT-Transformer model loaded successfully")
...
try:
    ...
    logger.info("SAINT model loaded successfully")
...
comparison_df.to_csv('./Section2_Model_Training/bike_sharing_model_comparison.csv', index=False)
print(f"\n💾 Comparison results saved to './Section2_Model_Training/bike_sharing_model_comparison.csv'")
...
# Save to pickle file
with open('./bike_sharing_section2_results.pkl', 'wb') as f:
    pickle.dump(section2_results, f)

print("✅ Section 2 results saved to './bike_sharing_section2_results.pkl'")
...
print(f"\n📁 Files Generated:")
print(f"   - Model comparison CSV: ./Section2_Model_Training/bike_sharing_model_comparison.csv")
...
print(f"   - Section 2 results pickle: ./bike_sharing_section2_results.pkl")
