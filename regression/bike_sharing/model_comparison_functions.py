"""
Model Comparison Functions for Bike Sharing Regression
...
def load_ft_transformer_model(model_dir='./Section2_Model_Training', feature_names=None, device='cpu'):
    """Load FT-Transformer model and results"""
    print("📦 Loading FT-Transformer model...")
    
    try:
        import rtdl
        
        # Load model checkpoint
        checkpoint = torch.load(f'{model_dir}/ft_transformer_model.pth', 
...
def load_improved_ft_transformer_model(model_dir='./Section2_Model_Training', feature_names=None, device='cpu'):
    ...
    try:
        ...
        print("✅ Improved FT-Transformer model loaded successfully!")
        return model_info
...
def load_saint_model(model_dir='./Section2_Model_Training', device='cpu'):
    """Load SAINT model and results"""
    print("📦 Loading SAINT model...")
    
    try:
        from saint_training_functions import SAINTModel
        
        # Load model data
        with open(f'{model_dir}/saint_model.pkl', 'rb') as f:
...
def load_all_models(model_dir='./Section2_Model_Training', feature_names=None, device='cpu'):
    ...
    if improved_ft_info:
        models['Improved FT-Transformer'] = improved_ft_info['model']
        model_results['Improved FT-Transformer'] = {
...
def save_comparison_results(comparison_df, predictions, feature_importance_data, 
    ...
    comparison_df.to_csv(f'{save_dir}/bike_sharing_model_comparison.csv', index=False)
...
def run_complete_model_comparison(data_path='./bike_sharing_preprocessed_data.pkl',
