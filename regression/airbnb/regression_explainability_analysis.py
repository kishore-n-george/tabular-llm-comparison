"""
Regression Explainability Analysis for Airbnb Price Prediction Dataset
...
class RegressionExplainabilityAnalyzer:
        ...
        try:
            ...
            if LIME_AVAILABLE:
                results['lime_analysis'] = self._get_lime_analysis(
                    model, X_train, X_test, max_samples, model_name
                )
    ...
    def _get_baseline_metrics(self, model, X_train, X_test, y_train, y_test):
        ...
        try:
            ...
            if not model_is_trained and hasattr(model, 'fit'):
                ...
                if hasattr(model, 'get_params') and ('XGBoost' in str(type(model)) or 'xgboost' in str(type(model)).lower()):
                    try:
                        ...
                        print(f"        📋 XGBoost parameters: {list(params.keys())}")
                        
                        # Always create a copy and disable early stopping for explainability analysis
                        print(f"        🔧 Creating XGBoost copy without early stopping for explainability analysis")
                        from copy import deepcopy
                        ...
                        model = model_copy
                        print(f"        ✅ XGBoost model fitted successfully without early stopping")
    ...
    def _get_lime_analysis(self, model, X_train, X_test, max_samples: int, model_name: str = "Model"):
        ...
        try:
            ...
            # Create LIME explainer with better handling for encoded categorical features
            explainer = lime.lime_tabular.LimeTabularExplainer(
                X_train_sample,
                feature_names=self.feature_names,
                mode='regression',
                discretize_continuous=False,  # Disable discretization to avoid categorical encoding issues
                random_state=42
            )
            ...
            for i in range(n_explain):
                try:
                    ...
                    # Save LIME plot with model name
                    fig = explanation.as_pyplot_figure()
                    fig.savefig(f'{self.save_dir}/{model_name}_lime_explanation_{i+1}.png', 
    ...
    def generate_explanation_report(self, model_name: str):
        ...
        try:
            ...
            if 'baseline_metrics' in results:
                ...
                # Format with safe handling of string vs numeric values
                print(f"   R² Score: {r2_score:.4f}" if isinstance(r2_score, (int, float)) else f"   R² Score: {r2_score}")
                ...
                print(f"   Training Time: {train_time:.3f}s" if isinstance(train_time, (int, float)) else f"   Training Time: {train_time}s")
                print(f"   Inference Time: {inference_time:.3f}s" if isinstance(inference_time, (int, float)) else f"   Inference Time: {inference_time}s")
