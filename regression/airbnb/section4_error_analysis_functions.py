...
including XGBoost, FT-Transformer, and SAINT on the Airbnb dataset.
...
def load_regression_models_and_data():
    ...
    try:
        ...
        try:
            with open('./airbnb_section2_results.pkl', 'rb') as f:
    ...
    for model_name, model in models.items():
        ...
        try:
            if 'XGBoost' in model_name:
                ...
                # SAINT predictions - handle target scaling
                model.eval()
                with torch.no_grad():
                    ...
                    try:
                        model_path = './Section2_Model_Training/improved_saint_model.pkl'
                        if os.path.exists(model_path):
                            with open(model_path, 'rb') as f:
                                saint_data = pickle.load(f)
                            if 'target_scaler' in saint_data and saint_data['target_scaler'] is not None:
                                target_scaler = saint_data['target_scaler']
...
def generate_business_insights(error_metrics, predictions, feature_error_analysis, feature_names):
    ...
    print(f"   - Explains {best_r2*100:.1f}% of variance in Airbnb price prediction")
    ...
    print(f"\n📋 Business Recommendations:")
    print(f"   1. Deploy {best_model} for production Airbnb price prediction")
    print(f"   2. Monitor prediction accuracy especially for extreme price ranges")
    ...
    print(f"   - Better predictions lead to improved pricing strategies and revenue optimization")
    print(f"   - Reduced pricing errors through data-driven price recommendations")
...
def save_error_analysis_results(error_metrics, predictions, feature_error_analysis, comparison_df, business_insights, feature_names):
    ...
    for model_name, analysis in feature_error_analysis.items():
        if len(analysis['significant_features']) > 0:
            feature_analysis_df = pd.DataFrame({
                ...
                'Feature_Name': [feature_names[i] if i < len(feature_names) else f"Unknown_{i}" for i in range(len(analysis['feature_differences']))],
    ...
    with open('./Section4_ErrorAnalysis/business_insights.txt', 'w') as f:
        f.write("AIRBNB PRICE PREDICTION - ERROR ANALYSIS INSIGHTS\n")
        f.write("=" * 60 + "\n\n")
...
def run_complete_error_analysis(section2_results_file='./airbnb_section2_results.pkl',
    ...
    try:
        ...
        results_dir = save_error_analysis_results(
            ...
            comparison_df, business_insights, data['feature_names']
        )
...
def compare_error_patterns_across_models(error_metrics, predictions, y_test):
    ...
    for model_name in predictions.keys():
        ...
        patterns = {
            ...
            'high_price_error': np.mean(np.abs(residuals[y_test > np.percentile(y_test, 75)])),
            'low_price_error': np.mean(np.abs(residuals[y_test < np.percentile(y_test, 25)])),
            'medium_price_error': np.mean(np.abs(residuals[
        ...
        print(f"     High price error: ${patterns['high_price_error']:.2f}")
        print(f"     Low price error: ${patterns['low_price_error']:.2f}")
...
def analyze_temporal_error_patterns(error_metrics, predictions, y_test, time_features=None):
    ...
    for model_name in predictions.keys():
        ...
        print(f"   {model_name}:")
        print(f"     Low prediction range error: ${temporal_analysis[model_name]['low_prediction_error']:.2f}")
        print(f"     Medium prediction range error: ${temporal_analysis[model_name]['medium_prediction_error']:.2f}")
        print(f"     High prediction range error: ${temporal_analysis[model_name]['high_prediction_error']:.2f}")
